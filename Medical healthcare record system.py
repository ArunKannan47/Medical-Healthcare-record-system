...
class Database:
    def __init__(self):
        ...
        self.dbCursor.execute("CREATE TABLE IF NOT EXISTS patient_info (id PRIMARYKEY text, fName text, lName text, dob text, mob text, yob text, gender text, address text, phone text, email text, bloodGroup text, history text, doctor text, covidtest text, covidvaccine text)")

    def __del__(self):
        self.dbCursor.close()
        self.dbConnection.close()

    def Insert(self, id, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, covidtest, covidvaccine):
        self.dbCursor.execute("INSERT INTO patient_info VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (id, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, covidtest, covidvaccine))
        self.dbConnection.commit()
        
    def Update(self, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, covidtest, covidvaccine, id):
        self.dbCursor.execute("UPDATE patient_info SET fName = ?, lName = ?, dob = ?, mob = ?, yob = ?, gender = ?, address = ?, phone = ?, email = ?, bloodGroup = ?, history = ?, doctor = ?, covidtest = ?, covidvaccine = ? WHERE id = ?", (fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, covidtest, covidvaccine, id))
        self.dbConnection.commit()
...
class Values:
    def Validate(self, id, fName, lName, phone, email, history, doctor, covidtest, covidvaccine):
        if not (id.isdigit() and (len(id) == 3)):
            ...
            return "doctor"
        elif not (covidtest.isalpha()):
            return "covidtest"
        elif not (covidvaccine.isalpha()):
            return "covidvaccine"    
        else:
            return "SUCCESS"
...
class InsertWindow:
    def __init__(self):
        ...
        self.covidtest = tkinter.StringVar()
        self.covidvaccine = tkinter.StringVar()
        ...
        tkinter.Label(self.window, text = "COVID test result",  width = 25).grid(pady = 5, column = 1, row = 14)
        tkinter.Label(self.window, text = "COVID vaccination status",  width = 25).grid(pady = 5, column = 1, row = 15)
        ...
        self.covidtestEntry = tkinter.Entry(self.window,  width = 25, textvariable = self.covidtest)
        self.covidvaccineEntry = tkinter.Entry(self.window,  width = 25, textvariable = self.covidvaccine)
        ...
        self.covidtestEntry.grid(pady = 5, column = 3, row = 14)
        self.covidvaccineEntry.grid(pady = 5, column = 3, row = 15)
    ...
    def Insert(self):
        ...
        self.test = self.values.Validate(self.idEntry.get(), self.fNameEntry.get(), self.lNameEntry.get(), self.phoneEntry.get(), self.emailEntry.get(), self.historyEntry.get(), self.doctorEntry.get(), self.covidtestEntry.get(), self.covidvaccineEntry.get())
        if (self.test == "SUCCESS"):
            self.database.Insert(self.idEntry.get(), self.fNameEntry.get(), self.lNameEntry.get(), self.dobBox.get(), self.mobBox.get(), self.yobBox.get(), self.genderBox.get(), self.addressEntry.get(), self.phoneEntry.get(), self.emailEntry.get(), self.bloodGroupBox.get(), self.historyEntry.get(), self.doctorEntry.get(), self.covidtestEntry.get(), self.covidvaccineEntry.get())
    ...
    def Reset(self):
        ...
        self.covidtestEntry.delete(0, tkinter.END)
        self.covidvaccineEntry.delete(0, tkinter.END)
...
class UpdateWindow:
    def __init__(self, id):
        ...
        # Button widgets
        tkinter.Button(self.window, width = 20, text = "Update", command = self.Update).grid(pady = 15, padx = 5, column = 1, row = 16)
        tkinter.Button(self.window, width = 20, text = "Reset", command = self.Reset).grid(pady = 15, padx = 5, column = 2, row = 16)
        tkinter.Button(self.window, width = 20, text = "Close", command = self.window.destroy).grid(pady = 15, padx = 5, column = 3, row = 16)
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView["columns"] = ("id", "fName", "lName", "dob", "mob", "yob", "gender", "address", "phone", "email", "bloodGroup", "history", "doctor","covidtest","covidvaccine")
        ...
        self.databaseView.heading("covidtest", text = "COVID test result")  
        self.databaseView.heading("covidvaccine", text = "COVID vaccination status")   
        ...
        self.databaseView.column("covidtest", width = 100)   
        self.databaseView.column("covidvaccine", width = 200) 
...
class HomePage:
    def __init__(self):
        ...
        self.homePageWindow.wm_title("Medical healthcare record system")
